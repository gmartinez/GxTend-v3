...
class Accendo_Git {
    ...
    public function issuecommand($command, $repopath, $worktree) {
        ...
        #
        $ACCENDO_BE_HOME = Accendo_System::get_eva("ACCENDO_BE_HOME");

        # Set git environment variables...
        if (file_exists($repopath) && ($worktree===null || $worktree==="null" || file_exists($worktree))) {
            #
            putenv("GIT_DIR=$repopath");
            if ($worktree!==null) { Accendo_System::get_eva("GIT_WORK_TREE", $worktree); }
            #
            Accendo_System::get_eva("GIT_AUTHOR_NAME", "Unknown");
            Accendo_System::get_eva("GIT_AUTHOR_EMAIL", "Unknown");
            Accendo_System::get_eva("GIT_COMMITTER_NAME", Accendo_Session::getSessVar("MyProfile.userdata.username"));
            Accendo_System::get_eva("GIT_COMMITTER_EMAIL", Accendo_Session::getSessVar("MyProfile.userdata.email"));
            #
            list($cmdsts, $cmdout) = Accendo_System::shell_exec("\"$ACCENDO_BE_HOME/Git/bin/git.exe\" $command");
        ...
        $env = array(
                    "ACCENDO_BE_HOME"=>$ACCENDO_BE_HOME,
    ...
    public function repoStats($repopath, $worktree, $commits_per_branch=0, $objects_per_commit=0) {
        ...
        if (!file_exists($repopath."/HEAD")) {
            ...
            if (!file_exists($repopath."/status")) {
                ...
                $repo_stats["ava"] = trim(basename(Accendo_File::read($repopath."/status")));
            }
            ...
            if ($commits_per_branch!==0) {
                ...
                foreach (Accendo_File::find("(.*)", $repopath, 1) as $fl) {
                ...
                if ($giticmd["sts"]==0) {
                    ...
                    if ($branchName && $branchName!="(no branch)") {
                        if (stripos($branchName, "remotes/origin/HEAD")!==false) {
                            ...
                            $i = 0;
                            $history = Accendo_Cvs::whatchanged("", $branchName, $repopath);
                            ...
                            if ($branchName == Accendo_Session::getSessVar("MyProfile.userdata.username")) {
